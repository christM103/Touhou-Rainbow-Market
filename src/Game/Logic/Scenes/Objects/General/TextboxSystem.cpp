...
void TR::TextboxSystem::initTextbox(Engine::ComponentManager* componentManager, Engine::Window* windowManager) {
...
void TR::TextboxSystem::updateEntity(const Engine::EntityManager* entityManager, Engine::ComponentManager* componentManager, Engine::Window* windowManager) {
	...
	if (auto* entities = componentManager->allEntities<TextBoxComponent>()) {
		if (entities->size() > _textboxes.size()) {
			this->initTextbox(componentManager, windowManager);
		}
	}
}
...
void TR::TextboxSystem::updateRender(const Engine::EntityManager* entityManager, Engine::ComponentManager* componentManager, Engine::Window* windowManager) {
	...
	Engine::TransformComponent* _transform_comp;
	...
	for (const auto& [entity, textbox] : _textboxes) {
		...
		_transform_comp = componentManager->getComponent<Engine::MultiTransformComponent>(entity)->transforms.at(0).get();
		...
		// The exit transition is active
			...
			switch (uint16_t temp = (textbox->_textbox_flags & movement_flags)) {
				case TextBoxComponent::TRANSITION_LEFT:
					hor = -4.0;
		...
		// The textbox is static
			...
			switch (uint16_t state = (textbox->_textbox_flags & textbox_states)) {
				case TextBoxComponent::TXT_CONTINUE:
					if (_input_comp->keyPressed(SDL_SCANCODE_SPACE)) {
						_text_comp->setText("");
						textbox->textLine = 0;
...
void TR::TextboxSystem::create(const Engine::SystemContext& ctx) {
	if (!ctx.entityManager || !ctx.componentManager || !ctx.window) {
		...
		this->initTextbox(ctx.componentManager, ctx.window);
	}
}
...
void TR::TextboxSystem::update(const Engine::SystemContext& ctx) {
	if (!ctx.entityManager || !ctx.componentManager || !ctx.window) {
		...
		this->updateEntity(ctx.entityManager, ctx.componentManager, ctx.window);
