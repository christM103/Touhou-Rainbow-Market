...
void TR::TurnSystem::initTurns(Engine::ComponentManager* componentManager) {
...
void TR::TurnSystem::initRender(Engine::ComponentManager* componentManager, Engine::Window* windowManager, Engine::AssetManager* assetManager) {
...
void TR::TurnSystem::updateEntites(Engine::ComponentManager* componentManager, Engine::Window* windowManager, Engine::AssetManager* assetManager) {
	...
	if (active_entity.first == nullptr) {
		if (auto* entities = componentManager->allEntities<TurnsComponent>()) {
			this->create(Engine::SystemContext(nullptr, componentManager, assetManager, nullptr, windowManager));
		}
	}
}

void TR::TurnSystem::updateTurns(Engine::ComponentManager* componentManager) {
	if (active_entity.first != nullptr) {
...
void TR::TurnSystem::updateRender(Engine::ComponentManager* componentManager, Engine::Window* windowManager) {
	if (active_entity.first != nullptr) {
		auto active_turn = active_entity.second->turns.at(active_entity.second->current_turn);
...
void TR::TurnSystem::create(const Engine::SystemContext& ctx) {
	if (!ctx.entityManager || !ctx.componentManager || !ctx.window || !ctx.assetManager) {
		...
		this->initTurns(ctx.componentManager);
		if (ctx.entityManager->entityExists("BGO")) {
			this->initRender(ctx.componentManager, ctx.window, ctx.assetManager);
		}
	}
}
...
void TR::TurnSystem::update(const Engine::SystemContext& ctx) {
	if (!ctx.entityManager || !ctx.componentManager || !ctx.window || !ctx.assetManager) {
		...
		this->updateTurns(ctx.componentManager);
		if (ctx.entityManager->entityExists("BGO")) {
